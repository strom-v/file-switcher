...
if (!gotLock) {
  ...
  app.whenReady().then(() => {
    ...
    // (например приложение упало), возвращаем прокси в исходное состояние сейчас,
    // до того как пользователь успеет запустить новый прокси или столкнётся с потерей сети
